...
/// Always floors a cash value to the nearest cent (2 d.p.).
/// Never rounds up — prevents fractional-cent accumulation on the server.
double _floorToCents(double value) => (value * 100).floorToDouble() / 100;
...
class PortfolioService {
  ...
  Future<TradeResult> buy({
    ...
    final newCash = _floorToCents(user.cashBalance - actualCost);
  ...
  Future<TradeResult> sell({
    ...
    final newCash = _floorToCents(user.cashBalance + flooredRevenue);
  ...
  Future<TradeResult> deposit({required User user, required double amount}) async {
    ...
    await _userRepo.updateFinancials(
      userId: user.id,
      cashBalance: _floorToCents(user.cashBalance + amount),
      xp: newXp,
      level: newLevel,
    );
