...
class PortfolioNotifier extends StateNotifier<PortfolioState> {
  ...
  /// Keeps the history tab in sync after any write operation.
...
final portfolioProvider =
  ...
  // Reload when auth becomes authenticated, OR when the signed-in user changes
  // (e.g. guest → logged-in account: both states are authenticated so the
  // first condition alone would never fire for that transition).
  ref.listen(authProvider, (prev, next) {
    final justAuthenticated =
        next.isAuthenticated && !(prev?.isAuthenticated ?? false);
    final userSwitched = next.isAuthenticated &&
        (prev?.isAuthenticated ?? false) &&
        next.user?.id != prev?.user?.id &&
        next.user != null;
    if (justAuthenticated || userSwitched) {
      notifier.loadAll();
    }
  });
  return notifier;
